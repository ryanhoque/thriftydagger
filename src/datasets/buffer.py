...
class BufferDataset(Dataset):
    def __init__(self, data, max_size=MAX_BUFFER_SIZE, shuffle=True, split="train", train_perc=0.9) -> None:
        ...
        self.ptr = 0
        self.obs, self.act, self.curr_size = self.split_data(data, split, train_perc, shuffle=shuffle)

    def split_data(self, data, split, train_perc, shuffle) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        obs = data["obs"]
        act = data["act"]
        ...
        if split == "train":
            ...
            raise ValueError(f"split must be one of {{'train', 'val'}}; got split='{split}' instead!")
