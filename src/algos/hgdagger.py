...
class HGDagger(BaseAlgorithm):
    def __init__(
        self, model, save_dir, max_traj_len, device, lr=1e-3, optimizer=torch.optim.Adam, is_ensemble=True
    ) -> None:

        super().__init__(model, save_dir, max_traj_len, device, lr=lr, optimizer=(None if is_ensemble else optimizer))

        self.is_ensemble = is_ensemble
        if self.is_ensemble:
            self.ensemble_optimizers = [
                optimizer(self.model.models[i].parameters(), lr=lr) for i in range(len(self.model.models))
            ]
